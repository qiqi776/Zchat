...
type UserContact struct {
	Id          int64     `gorm:"column:id;comment:自增id"`
	UserId      string    `gorm:"column:user_id;type:char(20);not null;comment:用户唯一id"`
	ContactId   string    `gorm:"column:contact_id;type:char(20);not null;comment:对应联系id"`
	ContactType bool      `gorm:"column:contact_type;type:tinyint(1);not null;comment:联系类型"`
